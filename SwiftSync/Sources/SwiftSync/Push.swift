...
/// Result of a push pass. SwiftSync advances its own per-type cursor internally on a fully-acknowledged
/// push, so there is no cursor for the caller to persist — just the counts and any per-row failures.
...
extension SwiftSync {
    ...
    /// per-type cursor: transactions authored by anyone other than the inbound (pull) author. A row
    ...
    public static func pendingChanges<Model: SyncUpdatableModel>(
        ...
        return try pendingChanges(for: Model.self, in: context, since: storedCursor(for: Model.self, in: context))
    }
    ...
    /// Drive one push: read pending changes from history (since SwiftSync's stored per-type cursor),
    /// hand their ids to the app's `upload` closure (the app owns the network call), then — only when
    /// *every* pending change was acknowledged — advance the stored cursor to the latest token and trim
    /// the now-redundant inbound history. SwiftSync writes no per-row state on push; an unacknowledged
    /// change simply stays past the cursor and is re-detected next push.
    @discardableResult
    public static func push<Model: SyncUpdatableModel>(
        ...
        try requireOfflineCapable(Model.self, in: context)
        let cursor = storedCursor(for: Model.self, in: context)
        let pending = try pendingChanges(for: Model.self, in: context, since: cursor)
        ...
        if acknowledgedEverything, let latest = try latestToken(in: context) {
            try storeCursor(latest, for: Model.self, in: context)
    ...
    /// history since the stored cursor. The pull uses this to preserve never-pushed local inserts from
    ...
    static func locallyDirtyPersistentIDs<Model: SyncUpdatableModel>(
        ...
        let transactions = try localTransactions(since: storedCursor(for: Model.self, in: context), in: context)
        ...
        -> [DefaultHistoryTransaction]
    {
        ...
        if let cursor {
            descriptor.predicate = #Predicate {
                $0.token > cursor && ($0.author == nil || $0.author != inbound)
            }
        } else {
