...
final class SyncFeatureInteropTests: XCTestCase {
    ...
    func testIndexOnNonIdentityFieldIsTransparentToSync() async throws {
        let context = ModelContext(try ModelContainer(for: InteropIndexedNote.self, configurations: ModelConfiguration(isStoredInMemoryOnly: true)))
        ...
        XCTAssertEqual(
            notes.count,
            2,
            "#Index must not change row identity or count"
        )
        XCTAssertEqual(
            notes.first { $0.id == 1 }?.title,
            "A2",
            "upsert-by-id must still update in place"
        )
    }

    @MainActor
    func testUniqueOnNonIdentityFieldCausesSilentSyncDataLoss() async throws {
        XCTExpectFailure("Known incompatibility: a unique constraint on a non-identity property lets SwiftData's constraint-based upsert destroy identity-distinct rows during sync. Declare uniqueness only on the sync identity.")

        let context = ModelContext(try ModelContainer(for: InteropUniqueEmailUser.self, configurations: ModelConfiguration(isStoredInMemoryOnly: true)))
        ...
        XCTAssertEqual(
            users.count,
            2,
            "both identity-distinct rows should survive"
        )
    ...
    func testCompoundUniqueOnSyncedFieldsCausesSilentSyncDataLoss() async throws {
        XCTExpectFailure("Known incompatibility: compound #Unique on synced fields lets SwiftData destroy identity-distinct rows during sync. Declare uniqueness only on the sync identity.")

        let context = ModelContext(try ModelContainer(for: InteropCompoundUniqueEvent.self, configurations: ModelConfiguration(isStoredInMemoryOnly: true)))
    ...
    func testSyncContainerAcceptsIndexAndIdentityUniqueModel() throws {
        XCTAssertNoThrow(try SyncContainer(for: InteropIndexedNote.self, configurations: ModelConfiguration(isStoredInMemoryOnly: true)))
    }

    @MainActor
    func testSyncContainerRejectsUniqueOnNonIdentityField() throws {
        XCTAssertThrowsError(try SyncContainer(for: InteropUniqueEmailUser.self, configurations: ModelConfiguration(isStoredInMemoryOnly: true))) { error in
            XCTAssertTrue(
                {
    ...
    func testSyncContainerRejectsCompoundUniqueOnNonIdentityFields() throws {
        XCTAssertThrowsError(try SyncContainer(for: InteropCompoundUniqueEvent.self, configurations: ModelConfiguration(isStoredInMemoryOnly: true))) { error in
            XCTAssertTrue(
                {
                    if case SyncError.schemaValidation = error { return true }
                    return false
