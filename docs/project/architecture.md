...
**Dependency direction is downward only.** A view never imports a networking type; the engine never
reaches into SwiftData; SwiftSync never knows an HTTP request exists. Each arrow above is the *only* thing
a layer knows about the one below it.
