...
- Step 8 rollout completion: demo models now rely on convention mapping by default, keeping explicit mapping only for intentional backend key differences (e.g. `description` -> `descriptionText`)
