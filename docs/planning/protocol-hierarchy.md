...
- Done: **Delete `SyncRelationshipSchemaIntrospectable`** — `syncRelationshipSchemaDescriptors` folded into `SyncModelable` with a default `[]` implementation. The `SyncContainer` schema validation cast now targets `SyncModelable` directly. Net change: `-1 protocol, ~0 behavior change`.
